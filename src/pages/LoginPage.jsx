...
import { Eye, EyeOff, Layers, Star, StarHalf, Info, Shield, HelpCircle, CheckCircle2, ArrowRight } from 'lucide-react'
...
export default function LoginPage() {
  ...
  const { signIn, signUp, signInWithGoogle, session, ready, error, clearError } = useAuthStore()
  ...
  const switchMode = (newMode) => {
    setMode(newMode); clearError(); setFieldErrors({})
    setForm({ fullName: '', email: '', password: '', confirm: '' }); setShowPass(false)
  }
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row font-sans antialiased">
      ...
      >
        ...
        <div className="lg:hidden relative flex flex-col h-[280px] sm:h-[340px] overflow-hidden">
          ...
          <div className="absolute inset-0"
            style={{ background: 'linear-gradient(to top, #0a0f1a 15%, rgba(10,15,26,0.55) 60%, rgba(10,15,26,0.3) 100%)' }} />
          ...
          <div className="relative z-10 flex items-center gap-2.5 p-6">
            ...
            <span className="text-[18px] font-extrabold tracking-tight text-white">Work OS</span>
          </div>
          ...
          <div className="relative z-10 flex-1 flex flex-col justify-end px-6 pb-6">
            <h1 className="text-[1.8rem] sm:text-[2.1rem] font-extrabold text-white leading-[1.06] tracking-[-0.02em]">
              Gestión más{' '}
              <span style={{ color: '#34d399' }}>inteligente</span>
              <br />con Work OS
            </h1>
        ...
        >
          ...
          <div className="flex items-center gap-2 pb-3">
            ...
            <span className="text-[17px] font-extrabold tracking-tight text-white">Work OS</span>
          </div>
          ...
          <div className="flex flex-col justify-center overflow-y-auto min-h-0 pr-1">
            ...
            <h1 className="font-extrabold text-white leading-[1.05] tracking-[-0.03em] mb-3"
            ...
            <p className="mb-5 max-w-[340px]"
              style={{ fontSize: 'clamp(12px, 1vw, 14px)', color: 'rgba(255,255,255,0.5)', lineHeight: '1.6' }}>
              Un espacio unificado para planificar, ejecutar y entregar con tu equipo.
            </p>

            {/* Features — spacing apretado para 14" */}
            <ul className="space-y-1.5 mb-5">
              {['Tableros Kanban, Gantt y Listas', 'Colaboración en tiempo real', 'Reportes e informes automáticos'].map((f) => (
                <li key={f} className="flex items-center gap-2"
                  style={{ fontSize: 'clamp(11px, 0.9vw, 13px)', color: 'rgba(255,255,255,0.6)' }}>
            ...
            <div className="relative w-full max-w-[440px] hidden xl:block">
              <div className="rounded-2xl overflow-hidden"
                ...
                <div className="flex items-center gap-2 px-3 py-2"
                  ...
                  <div className="flex-1 mx-2 h-4 rounded-md flex items-center justify-center text-[9px]"
            ...
            <div className="mt-5 flex items-center gap-3 flex-wrap">
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-full"
                ...
                <div className="flex text-yellow-400">
                  {[...Array(4)].map((_, i) => <Star key={i} fill="currentColor" size={11} />)}
      ...
      <div className="flex flex-col lg:w-1/2 bg-white">
        ...
        <div className="flex items-center justify-end px-8 py-5 shrink-0"
            ...
            {isLogin ? (
              <>
                ...
                <button type="button" onClick={() => switchMode('register')}
                  className="font-bold transition-colors" style={{ color: '#10b981' }}>
                  Regístrate gratis
                </button>
              </>
            ) : (
        ...
        <div className="flex-1 flex items-center justify-center px-8 py-10 lg:py-8">
          <div className="w-full" style={{ maxWidth: '380px' }}>
            ...
            <AnimatePresence mode="wait">
              <motion.div key={`hdr-${mode}`}
                initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.15 }}
                className="mb-7">
                <h2 className="font-extrabold tracking-tight mb-1.5"
                  style={{ fontSize: 'clamp(1.4rem, 2vw, 1.75rem)', color: '#0f172a', letterSpacing: '-0.025em' }}>
            ...
            {/* Tabs */}
            <div className="relative flex rounded-xl p-1 mb-6" style={{ background: '#f1f5f9' }}>
              {[{ id: 'login', label: 'Iniciar sesión' }, { id: 'register', label: 'Crear cuenta' }].map(({ id, label }) => (
                <button key={id} type="button" onClick={() => switchMode(id)}
                  ...
                  {mode === id && (
                    <motion.div layoutId="form-tab"
                      className="absolute inset-0 rounded-[10px]"
                      style={{ background: '#fff', boxShadow: '0 1px 4px rgba(0,0,0,0.08), 0 0 0 1px rgba(0,0,0,0.04)' }}
                      transition={{ type: 'spring', stiffness: 500, damping: 35 }} />
                  )}
                  <span className={`relative ${mode === id ? 'text-slate-800' : 'text-slate-400 hover:text-slate-500'}`}>
                    {label}
                  </span>
                </button>
              ))}
              ...
              <motion.div key={mode}
                ...
                <form onSubmit={handleSubmit} className="space-y-3.5">
                  ...
                  {!isLogin && (
                    <div>
                      ...
                      <input type="text" name="fullName" value={form.fullName}
                        ...
                        className="w-full h-[46px] rounded-xl px-4 text-[14px] font-medium outline-none transition-all"
                        style={inputBase(fieldErrors.fullName)}
                        onFocus={onFocusInput}
                        onBlur={(e) => onBlurInput(e, fieldErrors.fullName)}
                      />
                      {fieldErrors.fullName && <p className="text-[12px] mt-1.5" style={{ color: '#ef4444' }}>{fieldErrors.fullName}</p>}
                    </div>
                  )}
                  ...
                  <div>
                    ...
                    />
                    {fieldErrors.email && <p className="text-[12px] mt-1.5" style={{ color: '#ef4444' }}>{fieldErrors.email}</p>}
                  </div>

                  {/* Contraseña */}
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      <label className="text-[12px] font-semibold" style={{ color: '#64748b' }}>Contraseña</label>
                      {isLogin && (
                        <Link to="/forgot-password" className="text-[12px] font-semibold transition-colors"
                          style={{ color: '#94a3b8' }}>
                          ¿Olvidaste la tuya?
                        </Link>
                      )}
                    ...
                    <div className="relative">
                      <input type={showPass ? 'text' : 'password'} name="password" value={form.password}
                        ...
                        className="w-full h-[46px] rounded-xl px-4 pr-12 text-[14px] font-medium outline-none transition-all"
                    ...
                    {fieldErrors.password && <p className="text-[12px] mt-1.5" style={{ color: '#ef4444' }}>{fieldErrors.password}</p>}
                  </div>
                  ...
                  {!isLogin && (
                    <div>
                      ...
                      />
                      {fieldErrors.confirm && <p className="text-[12px] mt-1.5" style={{ color: '#ef4444' }}>{fieldErrors.confirm}</p>}
                    </div>
                  )}

                  {/* Submit */}
                  <button type="submit" disabled={submitting}
                    className="!mt-5 w-full h-[50px] font-bold rounded-xl text-[15px] text-white
                    ...
                    onMouseEnter={e => { if (!submitting) { e.currentTarget.style.boxShadow = '0 6px 22px rgba(16,185,129,0.4)'; e.currentTarget.style.transform = 'translateY(-1px)' } }}
                    onMouseLeave={e => { e.currentTarget.style.boxShadow = '0 4px 16px rgba(16,185,129,0.28)'; e.currentTarget.style.transform = 'translateY(0)' }}>
                    {submitting ? (
                      <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                    ) : (
                      <>
                        {isLogin ? 'Ingresar a mi cuenta' : 'Crear cuenta gratis'}
                        <ArrowRight size={16} />
                      </>
                    )}
                  </button>
                ...
                <div className="flex items-center gap-3 my-5">
                  ...
                  <span className="text-[11px] font-semibold uppercase tracking-wider" style={{ color: '#cbd5e1' }}>o</span>
                  <div className="flex-1 h-px" style={{ background: '#f1f5f9' }} />
                </div>

                {/* Google */}
                <button type="button" onClick={handleGoogle}
                  className="w-full flex items-center justify-center gap-2.5 h-[46px] rounded-xl
                             font-semibold text-[14px] transition-all focus:outline-none"
                  style={{ border: '1.5px solid #e2e8f0', background: '#fff', color: '#334155' }}
                  onMouseEnter={e => { e.currentTarget.style.border = '1.5px solid #cbd5e1'; e.currentTarget.style.background = '#f8fafc' }}
                ...
                {isLogin ? (
                  <div className="mt-5 flex flex-wrap items-center justify-center gap-x-4 gap-y-2">
                    ...
                    <span className="flex items-center gap-1.5 text-[12px]" style={{ color: '#94a3b8' }}>
                      ...
                      <span className="font-bold" style={{ color: '#475569' }}>13.000+ reseñas</span>
                    </span>
                  </div>
                ) : (
                  <p className="mt-5 text-[12px] text-center leading-relaxed" style={{ color: '#94a3b8' }}>
                    Al crear tu cuenta aceptas los{' '}
                    <a href="mailto:privacidad@workos.app" style={{ color: '#10b981' }} className="font-semibold hover:underline">Términos de uso</a>
                    {' '}y la{' '}
                    <a href="mailto:privacidad@workos.app" style={{ color: '#10b981' }} className="font-semibold hover:underline">Política de privacidad</a>.
                  </p>
                )}
        ...
        <div className="lg:hidden px-8 py-4 flex items-center justify-center gap-4 shrink-0"
          style={{ borderTop: '1px solid #f1f5f9' }}>
          <span className="text-[11px]" style={{ color: '#cbd5e1' }}>© 2025 Work OS</span>
          <a href="mailto:privacidad@workos.app" className="text-[11px] flex items-center gap-1 hover:text-slate-500 transition-colors" style={{ color: '#cbd5e1' }}>
            <Shield size={10} /> Privacidad
          </a>
          <a href="mailto:soporte@workos.app" className="text-[11px] flex items-center gap-1 hover:text-slate-500 transition-colors" style={{ color: '#cbd5e1' }}>
