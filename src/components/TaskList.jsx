...
const STATUS_CONFIG = {
  ...
  pending: { label: 'Pendiente', bg: 'bg-gray-300', text: 'text-white' },
  completed: { label: 'Listo', bg: 'bg-emerald-500', text: 'text-white' },
};
...
function AvatarCircle({ name }) {
  ...
  return (
    <div className={`w-7 h-7 rounded-full ${colors[colorIdx]} flex items-center justify-center text-white font-bold text-[10px]`}>
      {initials}
    </div>
  );
}

export default function TaskList({ projects, toggleTask, deleteTask, updateTask }) {
  ...
  const statsLocal = {
    ...
    listo: allTasks.filter((t) => t.status === 'listo').length,
    en_curso: allTasks.filter((t) => t.status === 'en_curso').length,
    detenido: allTasks.filter((t) => t.status === 'detenido').length,
    overdue: allTasks.filter((t) => !t.completed && t.dueDate && new Date(t.dueDate + 'T23:59:59') < new Date()).length,
  };
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 px-4 py-2.5 flex items-center gap-2 flex-wrap">
        <div className="relative flex-1 max-w-xs">
          ...
          <input
            ...
            className="w-full pl-8 pr-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-shatter-accent"
          />
        </div>
        ...
        <select
          ...
          className="px-3 py-1.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-shatter-accent"
        >
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="text-left text-xs font-semibold text-gray-600 px-3 py-2.5 min-w-[200px]">Tarea</th>
                <th className="text-left text-xs font-semibold text-gray-600 px-3 py-2.5 w-[140px]">Proyecto</th>
                <th className="text-center text-xs font-semibold text-gray-600 px-3 py-2.5 w-[100px]">Responsable</th>
                <th className="text-center text-xs font-semibold text-gray-600 px-3 py-2.5 w-[110px]">Estado</th>
                <th className="text-center text-xs font-semibold text-gray-600 px-3 py-2.5 w-[120px]">Vencimiento</th>
                <th className="text-center text-xs font-semibold text-gray-600 px-3 py-2.5 w-[140px]">Notas</th>
                <th className="text-center text-xs font-semibold text-gray-600 px-3 py-2.5 w-[130px]">Actualización</th>
                <th className="w-8"></th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((task) => {
                ...
                return (
                  <tr key={`${task.projectId}-${task.id}`} className="border-b border-gray-100 hover:bg-blue-50/30 group transition-colors">
                    ...
                    <td className="px-3 py-2.5">
                      <span className="text-xs text-shatter-accent font-medium flex items-center gap-1">
                    ...
                    <td className="px-3 py-2.5">
                      <div className="flex justify-center">
                        <span className={`${statusCfg.bg} ${statusCfg.text} px-4 py-1 rounded-sm text-xs font-semibold min-w-[90px] text-center inline-block`}>
                          {statusCfg.label}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-2.5 text-center">
                      <span className="text-xs text-gray-500 italic truncate block max-w-[130px]">
