  ...
  return (
    <div className="relative">
      <button
        ...
        className={`${config.bg} ${config.text} px-4 py-1 rounded-sm text-xs font-semibold min-w-22.5 text-center hover:opacity-90 transition-opacity`}
      >
        {config.label}
      </button>
      {open && (
        <>
          ...
          <div className="absolute top-full left-0 mt-1 bg-white rounded-lg shadow-xl border border-gray-200 py-1 z-20 min-w-30">
...
function TaskGroup({ title, color, tasks, projectId, onToggleTask, onUpdateTask, onDeleteTask, onAddTask, collapsed, onToggleCollapse }) {
  ...
  const statusCounts = useMemo(() =>
    tasks.reduce(
      ...
      { listo: 0, en_curso: 0, detenido: 0, pendiente: 0 }
    ),
    [tasks]
  );
  ...
  return (
    <div className="mb-6">
      <button onClick={onToggleCollapse} className="flex items-center gap-2 mb-1 group">
        {collapsed
            ...
            <thead>
              <tr className="bg-white border-b border-gray-200">
                ...
                <th className="w-8 px-2 py-2.5">
                  <input type="checkbox" className="rounded border-gray-300 text-app-accent focus:ring-app-accent" disabled />
                </th>
                <th className="text-left text-xs font-semibold text-gray-600 px-3 py-2.5 min-w-45">Tarea</th>
                <th className="text-left text-xs font-semibold text-gray-600 px-3 py-2.5 w-25"></th>
                <th className="text-center text-xs font-semibold text-gray-600 px-3 py-2.5 w-30">Responsable</th>
                <th className="text-center text-xs font-semibold text-gray-600 px-3 py-2.5 w-27.5">
                  Estado <AlertCircle className="w-3 h-3 inline text-gray-400 ml-0.5" />
                </th>
                <th className="text-center text-xs font-semibold text-gray-600 px-3 py-2.5 w-30">
                  Vencimiento <AlertCircle className="w-3 h-3 inline text-gray-400 ml-0.5" />
                </th>
                <th className="text-center text-xs font-semibold text-gray-600 px-3 py-2.5 w-40">Notas</th>
                <th className="text-center text-xs font-semibold text-gray-600 px-3 py-2.5 w-35">Última actualización</th>
                <th className="w-8 px-2 py-2.5">
                  ...
                  <td className="px-3 py-2">
                    {editingNotes === task.id ? (
                      <input
                        ...
                        onBlur={() => { onUpdateTask(projectId, task.id, { notes: notesValue }); setEditingNotes(null); }}
                        onKeyDown={(e) => {
                      ...
                      <span
                        ...
                        className="text-xs text-gray-500 cursor-pointer hover:text-gray-700 block truncate max-w-37.5 italic"
                      >
                  ...
                  {addingTask ? (
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        autoFocus
                      />
                      <button onClick={handleAddTask} className="p-1 text-emerald-500 hover:bg-emerald-50 rounded">
                        <Save className="w-4 h-4" />
                      </button>
                      <button onClick={() => { setAddingTask(false); setNewTaskTitle(''); }} className="p-1 text-gray-400 hover:bg-gray-100 rounded">
                        <X className="w-4 h-4" />
                      </button>
                    </div>
                  ) : (
                    <button
                      ...
                      className="text-sm text-gray-400 hover:text-app-accent transition-colors flex items-center gap-1"
                    >
            ...
            {tasks.length > 0 && (
              <tfoot>
                <tr className="bg-gray-50/50">
                  ...
                  <td></td>
                  <td className="px-3 py-2">
                    <div className="flex h-5 rounded-sm overflow-hidden min-w-22.5">
                      {statusCounts.listo    > 0 && <div className="bg-emerald-500" style={{ width: `${(statusCounts.listo    / totalTasks) * 100}%` }} />}
                      {statusCounts.en_curso > 0 && <div className="bg-amber-400"   style={{ width: `${(statusCounts.en_curso / totalTasks) * 100}%` }} />}
                      {statusCounts.detenido > 0 && <div className="bg-rose-500"    style={{ width: `${(statusCounts.detenido / totalTasks) * 100}%` }} />}
                      {statusCounts.pendiente> 0 && <div className="bg-gray-300"    style={{ width: `${(statusCounts.pendiente/ totalTasks) * 100}%` }} />}
                    </div>
                  </td>
                  <td className="px-3 py-2 text-center">
                    {dateRange && (
                      <span className="text-[10px] bg-app-accent/10 text-app-accent px-2 py-1 rounded-full font-medium">
                        {dateRange}
                      </span>
                    )}
                  </td>
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 px-4 py-2.5 flex items-center gap-1 flex-wrap">
        <button
          ...
          className="flex items-center gap-1.5 px-4 py-1.5 bg-app-accent hover:bg-app-accent/80 text-white rounded-md text-sm font-medium transition-colors mr-2"
        >
          Agregar tarea
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
        ...
        />
        <button className="flex items-center gap-2 text-sm text-gray-400 hover:text-app-accent mt-4 transition-colors">
