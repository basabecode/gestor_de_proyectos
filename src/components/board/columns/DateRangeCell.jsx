...
export default function DateRangeCell({ value, onChange }) {
  ...
  // Update local state when external value changes (controlled by parent)
  ...
  const handleChange = (field, val) => {
    const next = {
      start: field === 'start' ? val : start,
      end: field === 'end' ? val : end,
    }
    ...
    <div ref={anchorRef} className="relative w-full flex justify-center">
      ...
      >
        ...
        <div className="mb-3">
          ...
          <div className="relative">
            <input
              ...
              className="w-full h-8.5 px-3 text-[13px] border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary"
            />
            {start && (
              <button
        ...
        <button
          ...
          className="mt-3 w-full h-7.5 text-[12px] font-semibold text-white rounded-lg bg-primary hover:bg-primary-hover transition-colors"
        >
          Confirmar
        </button>
      </CellDropdown>
